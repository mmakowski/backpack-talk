...
\node[state,unix,below of=unixelf,node distance=9em](unixturntap){UnixTurntap};
...
\begin{tikzpicture}[
  ...
  unix/.style={fill=red!20},
  mir/.style={fill=green!20}]
\node[state,xen](xenboot){XenBoot:XEN};
\node[state,xen,below of=xenboot,xshift=-5em](xenstore){XenStore:XS(XEN)};
\node[state,xen,below of=xenboot,xshift=5em](xenevtchn){XenEvtchn:EVENT(XEN)};
\node[state,xen,below of=xenstore,xshift=5em](xenring){XenRing:RING(XS)(EVENT)};
\node[state,xen,below of=xenring](xennetif){XenNetif:NETIF(XEN)(RING)};
\node[state,mir,below of=xennetif,xshift=4.5em](mirnet){MirNet:ETH(NETIF)};
\node[state,mir,below of=mirnet](mirtcp){MirTCP:TCP(ETH)};
\node[state,mir,below of=mirtcp](cohttp){Cohttp:HTTP(TCP)};
\node[state,fill=gray!20,left of=cohttp,node distance=10em](myhomepage){
\begin{tabular}{l}
MyHomePage:\\
APP(HTTP)
\end{tabular}
};
\node[state,unix,right of=xenboot,node distance=12em](unixelf){UnixELF:UNIX};
\node[state,unix,below of=unixelf,node distance=7em](unixturntap){
\begin{tabular}{l}
	UnixTurntap:\\
	NETIF(UNIX)
\end{tabular}
